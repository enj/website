...
The client may optionally set the `spec.durationHint` to request a particular
...
1. **Expiration/certificate lifetime**: whether it is fixed by the signer, configurable by the admin, determined by the CSR object, etc
   and the behavior when an expiration is different than the signer-determined expiration that is specified in the CSR via the `spec.durationHint` field.
...
through the `spec.durationHint` field of the CSR object. The built-in signers
...
as the default when no `spec.durationHint` is specified.  When `spec.durationHint`
is specified, the minimum of `spec.durationHint` and `ClusterSigningDuration` is
used.

{{< note >}}
The `spec.durationHint` field was added in Kubernetes v1.22.  Consequently, earlier versions of Kubernetes will not honor this field.
{{< /note >}}
...
1. `kubernetes.io/kube-apiserver-client`: signs certificates that will be honored as client certificates by the API server.
   Never auto-approved by {{< glossary_tooltip term_id="kube-controller-manager" >}}.
    ...
    1. Expiration/certificate lifetime - set to the minimum of either the `--cluster-signing-duration` option for the
       kube-controller-manager implementation of this signer or the `spec.durationHint` field of the CSR object if specified.
...
- `durationHint` could be made longer (i.e. `10d`) or shorter (i.e. `1h`)
